...
$sql= mysqli_query($conn,"SELECT * FROM `logs` WHERE `req_send_date` = '".$today_date."' AND `hospital_id` = '".$id."' AND `flag` = '0'"); 
$result = mysqli_num_rows($sql);

$Reached_sql = mysqli_query($conn,"SELECT * FROM `logs` WHERE `req_send_date` = '".$today_date."' AND `hospital_id` = '".$id."' AND `flag` = '1' and `reached` = '0'"); 
$R_result = mysqli_num_rows($Reached_sql);

if(isset($_POST['ackn']))
{
  $case_no = $_POST['ackn']; 
   $sql = mysqli_query($conn, "UPDATE `logs` SET `req_accept_date` = '".$date."', `req_accept_time` = '".$time."', `flag`= '1' WHERE `case_id` ='".$case_no."' ");
    if($sql)
    {
    ...
    {
      echo"<script type='text/javascript'> 
            alert('Sorry error while Acknowledgement....!!');
        </script>"; 
    }
...
if(isset($_POST['admitted']))
  {
        ...
        $case_no = $_POST['admitted'];
        $time = date('H:i:s');
        $date = date('Y-m-d');

        $sql = mysqli_query($conn, "UPDATE `logs` SET `req_accept_date` = '".$date."', `req_accept_time` = '".$time."', `reached`= '1' WHERE `case_id` ='".$case_no."' ");
        if($sql)
        {
          echo"<script type='text/javascript'> 
                alert('Updated..!!');
...
<body>
            ...
            <div class="col-xl-4 col-lg-6">
              <div class="card card-stats mb-4 mb-xl-0">
                <div class="card-body">
                  ...
                  <p class="mt-3 mb-0 text-muted text-sm">
                    Click the below <b>Accepted</b> button when patient get admitted to your Hospital
                  </p>
                </div>
              </div>
            </div>
        ...
        <div class="col">
          <div class="card bg-default shadow">
            <div class="card-header bg-transparent border-0">
              ...
              <h3 class="text-white mb-0" style="margin-left: 10px;">TODAY'S REQUEST </h3>
              <h3 class="text-white mb-0" style="margin-left: 800px;">DATE - <?php echo $today_date; ?> </h3>
              </div>
            </div>
                          ...
                          $i=1;
                            while($row = mysqli_fetch_array($sql))
                           {
                             echo '
                              ...
                              <td>'.'<form name="myform" method="POST" action = "HA_Home.php">
                                  <button class="btn btn-danger my-3" onclick="Admitted()" type="submit" id="ackn" name="ackn" value='.$row["case_id"].'> Acknowledge
                                  </button></form>'.'</td>
                                   </tr>';
  ...
  <!-- Cases Reached -->
  <div class="container mt--10 pb-5" style="max-width: 1600px;">
    <div class="row justify-content-center">
      <div class="col-lg-12 col-md-8" style="width: 570px;">
        ...
        <div class="col">
          <div class="card bg-default shadow">
            <div class="card-header bg-transparent border-0">
              <div class="row">
              <h3 class="text-white mb-0" style="margin-left: 30px;">Cases Reached to Hospital </h3>
              ...
              <table class="table align-items-center table-dark table-flush" name="new_request_table">
                <thead class="thead-dark">
                  <tr>
                    ...
                    <th scope="col">Contact No</th>
                    <th scope="col">Ambulance No.</th>
                    <th scope="col">Patient Acceptance<br>Status</th>
                ...
                <tbody>
                      ...
                      {
                          $i=1;
                            while($row = mysqli_fetch_array($Reached_sql))
                           {
                             echo '
                              <tr>
                              <td>'.$i.'</td>
                              ...
                              <td>'.$row["amb_no_plate"].'</td>
                              <td>'.'<form name="myform" method="POST" action = "HA_Home.php">
                                  <button class="btn btn-danger my-3" onclick="Admitted()" type="submit" id="admitted" name="admitted" value='.$row["case_id"].'> Accept Patient
                                  </button></form>'.'</td>
                                   </tr>';
